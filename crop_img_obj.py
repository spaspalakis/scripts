...
def crop(img, data, green_x1, green_x2, green_y1, green_y2, diff_x, diff_y, left_list, top_list, crop_dir_folder,file_name ):
    
    print('\n----- Crop -----')

    ### cropped image ###   
    crop_img = img[green_y1:green_y2, green_x1:green_x2]  # green_top:green_bottom, green_left:green_right

    crop_h, crop_w, _ = crop_img.shape ; print(f'\ncrop_w: {crop_w}\ncrop_h: {crop_h}\n') # h,w for cropped image
    ...
    for i, dt in enumerate(data):
        ...
        # Here we subtract (x1,y1) of green box from (x1,y1) of every object bbox
        # The result should be the the initial coordinates (x1,y1) of every bbox in the cropped image 
        # Here is the whole magic , on how we correspond the coords of initial image (640,480) to a smaller size image.
        # The problem is to find the start point of every bbox in the cropped image
        crop_bbox_x1 =  abs(green_x1 - left_list[i]) 
        crop_bbox_y1 =  abs(green_y1 - top_list[i])  
        ...
        print(f'crop_bbox Left (x1): {crop_bbox_x1}')
        print(f'crop_bbox Top (y1): {crop_bbox_y1}\n')

        print(f'crop_bbox Right (x2): {crop_bbox_x2}')
        print(f'crop_bbox Bottom (y2): {crop_bbox_y2}\n')
        ...
        with open(f'{crop_dir_folder}/{file_name}_cropped.txt', 'a') as crop_txt:
            print(f'\ndt[0]: {dt[0]}')
            crop_txt.write(f'{dt[0]} {x} {y} {w} {h}\n')
    crop_txt.close()
        # cv2.rectangle(crop_img, (crop_bbox_x1, crop_bbox_y1), (crop_bbox_x2, crop_bbox_y2), (0, 0, 255), 1)
    
    
    #path =                 crop_folder dir    + image name  + extention        
    path = os.path.join(f'{crop_dir_folder}', file_name + '_cropped.jpg')
...
if __name__ == '__main__':
    ...
    FOLDER_DIR = '/home/stayros/Desktop/asd' #'/home/spaspalakis/Desktop/rembg_result'
    ...
    for file in  os.listdir(FOLDER_DIR):  #os.listdir(FOLDER_DIR):
        ...
        if file.endswith(".txt"): 
            ...
            fl = open(f'{FOLDER_DIR}/{file_name}.txt', 'r') # /home/spaspalakis/Desktop/rembg_result/0a9fe927701bb9ee.txt #/home/spaspalakis/Desktop/test_rembg3/
            data = fl.readlines() #; print(f'\ndata : {data}')
            fl.close()

            # function to find bboxes from initial image
            green_x1, green_x2, green_y1, green_y2, diff_x, diff_y, final_min_list, left_list, top_list  = display_boxes(img, data)

            ##############

            # directory where cropped images are stored. If not exist create folder with the name 'crop_folder'
            # crop_dir_folder = r'/home/spaspalakis/Desktop/crop_folder/'
            if not os.path.exists(CROP_DIR_FOLDER):
                os.makedirs(CROP_DIR_FOLDER)

            # function to crop image
            crop(img, data, green_x1, green_x2, green_y1, green_y2, diff_x, diff_y, left_list, top_list,CROP_DIR_FOLDER,file_name )
