...
def pad_func(file,input_jpg,pad_dir_folder,file_type,new_width,new_height):
    
    file_name = os.path.splitext(file)[0]   #; print(f'\nFile_name: {file_name}')

    # print(f'\nNew padding width: {new_width}\nNew padding height: {new_height}')

    padded_image,left_pad,top_pad = pad_image(input_jpg, new_width, new_height)
